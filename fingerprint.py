...
# ---- memory / speed cap ----
# A fingerprint query needs only ~10-15 s of audio for a perfect match.
# Reading just the first slice keeps RAM tiny on Streamlit Cloud (~1 GB)
# and makes decoding ~10x faster. Set to None to read the whole file
...
def smart_load(path_or_file, filename="", sr=SR, max_seconds=MAX_QUERY_SECONDS):
    """
    Load a QUERY clip for identification — capped to `max_seconds` (default 15)
    to keep memory and time low. Pass max_seconds=None to read the whole file.
    """
