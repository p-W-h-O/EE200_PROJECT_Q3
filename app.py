...
def load_db(path=DB_PATH):
    ...
    # ---- new payload format: {"db": {...}, "labels": [...]} ----
    if isinstance(payload, dict) and "db" in payload and "labels" in payload:
        return payload["db"], list(payload["labels"])

    # ---- payload format with a meta map: {"db": {...}, "meta": {label: count}} ----
    if isinstance(payload, dict) and "db" in payload and "meta" in payload:
        meta = payload["meta"]
        if isinstance(meta, dict) and meta:
            labels = sorted(map(str, meta.keys()))     # labels straight from meta
        else:
            labels = _labels_from_db(payload["db"])
        return payload["db"], labels

    # ---- a dict that is itself the db (keys look like hashes, not "db") ----
    if isinstance(payload, dict) and "db" in payload and not isinstance(payload.get("db"), dict):
        # "db" is actually a hash key, so the whole payload IS the db
        return payload, _labels_from_db(payload)

    # ---- bare dict format (notebook .pkl): hash -> [entry, ...] ----
    return payload, _labels_from_db(payload)


def _labels_from_db(db):
    """Extract the set of song labels from a hash->[entry] database, tolerant
    of entry shape: (time,label), (label,time), single label, or longer."""
    labels = set()
    for v in db.values():
        for entry in v:
            if isinstance(entry, (tuple, list)):
                if len(entry) == 2:
                    a, b = entry
                    lab = b if isinstance(b, str) else (a if isinstance(a, str) else b)
                    labels.add(lab)
                elif len(entry) == 1:
                    labels.add(entry[0])
                else:
                    # find the string element, else assume last
